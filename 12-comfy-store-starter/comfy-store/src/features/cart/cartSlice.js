...
const defaultState = {
  cartItems: [],
  numItemsInCart: 0,
  cartTotal: 0, //number of cart  items multiplied by specific price
  shipping: 500,
  tax: 0,
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    removeItem: (state, action) => {
      ...
      state.cartItems = state.cartItems.filter((i) => i.cartID !== cartID); //return  the items where the cartID is not matching thus filtering the item out
    ...
    editItem: (state, action) => {
      const { cartID, amount } = action.payload; // only editing the amount of a particular product is possible in the cart
