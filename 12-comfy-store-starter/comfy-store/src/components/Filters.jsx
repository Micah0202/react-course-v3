...
const Filters = () => {
  ...
  <Form className="bg-base-200 rounded-md px-8 py-4 grid gap-x-4 gap-y-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 items-center"> 
     ...
     <FormSelect label="select category" name="category" size='select-sm' list={meta.categories} defaultValue={category} />


     {/* SELECT COMPANIES DROPDOWN , name is what server is looking for , size='select-sm'  sets the size of the select drop down box */}
     <FormSelect label="select company" name="company" size='select-sm' list={meta.companies} defaultValue={company}  />


     {/* ORDER SORT A-Z ,Z-A ,HIGH ,LOW*/}
     <FormSelect label="sort by" name="order" size='select-sm' list={['a-z','z-a','high','low']}  defaultValue={order}/>
