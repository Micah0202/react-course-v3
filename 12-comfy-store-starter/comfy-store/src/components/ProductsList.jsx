//this is the ProductsLsit ie the list view of  the our products page 
  ...
  return (
    ...
    <div className="mt-12 grid gap-y-8">
        {products.map((product)=>{
            const {title,price , image,company }=product.attributes ; //id is not inside attributes
            //each product will be a Link 
            const dollarsAmount= formatPrice(price)
            return (
                <Link key={product.id} to={`/products/${product.id}`} className="p-8 rounded-lg flex flex-col sm:flex-row gap-y-4 flex-wrap bg-base-100 shadow-xl s hover:shadow-2xl duration-300 group ">
